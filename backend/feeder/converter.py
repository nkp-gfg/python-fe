...
def convert_multi_flight_availability(raw_data, airline, flight_number, departure_date, origin=""):
    """
    Convert MultiFlightRS into a normalized itinerary availability document.

    The response encodes class codes and values as CSV strings where positions map
    to each other (e.g. classes="Y,B,M" and values="9,6,2").
    """
    data = _strip_ns(raw_data)
    origin_destinations_raw = _ensure_list(data.get("OriginDestination", []))

    origin_destinations = []
    all_segments = []
    for od in origin_destinations_raw:
        itineraries_raw = _ensure_list(od.get("Itinerary", []))
        itineraries = []
        for itinerary in itineraries_raw:
            segments_raw = _ensure_list(itinerary.get("Segment", []))
            segments = []
            for seg in segments_raw:
                class_codes = _split_csv_tokens(seg.get("@classes", ""))
                status_values = _split_csv_tokens(seg.get("@values", ""))
                availability = []
                for idx, class_code in enumerate(class_codes):
                    seats = _safe_int(status_values[idx]) if idx < len(
                        status_values) else 0
                    availability.append({
                ...
                segment_doc = {
                    "segmentId": _safe_int(seg.get("@segmentId")),
                    "origin": seg.get("@origin", ""),
                    "destination": seg.get("@destination", ""),
                    "carrierCode": seg.get("@carrierCode", ""),
                    "flightNumber": str(seg.get("@flightNumber", "")),
                    "departureDate": seg.get("@departureDate", ""),
                    "returnCode": str(seg.get("@returnCode", "")),
                    "polled": str(seg.get("@polled", "N")).upper() == "Y",
                    "classes": class_codes,
                    "values": [_safe_int(v) for v in status_values],
                    "availabilityByClass": availability,
                }
        ...
        origin_destinations.append({
            ...
            "departureDate": od.get("@departureDate", ""),
            "itineraries": itineraries,
        })

    top_return_code = _safe_int(data.get("@returnCode"), default=-1)
    summary = {
        "segments": len(all_segments),
        "polledSegments": sum(1 for s in all_segments if s.get("polled")),
        "errorSegments": sum(1 for s in all_segments if str(s.get("returnCode", "")) not in ("", "0")),
        "availableClasses": sorted({
            av.get("classCode", "")
            for seg in all_segments
            for av in seg.get("availabilityByClass", [])
            if av.get("classCode")
        }),
    }
    ...
    return {
        ...
        "departureDate": departure_date,
        "fetchedAt": _now_iso(),
        "version": _safe_int(data.get("@version"), default=1),
        "returnCode": top_return_code,
        "returnMessage": data.get("@returnMessage", ""),
        "diagnostic": data.get("Diagnostic", ""),
        "originDestinations": origin_destinations,
        "summary": summary,
        "success": top_return_code == 0,
        "_raw": raw_data,
    }
